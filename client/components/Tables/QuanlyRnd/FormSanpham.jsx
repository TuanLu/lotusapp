...
const fetchConfig = {
  fetch: 'rnd/fetch',
  update: 'rnd/updateProduct',
  delete: 'rnd/deleteProduct/',
  changeStatus: 'rnd/changeStatus'
}
...
class EditableTable extends React.Component {
  ...
  addNewRow() {
    let {products} = this.props.mainState.rnd || [];
  ...
  save(form, key) {
    form.validateFields((error, row) => {
      ...
      const newData = [...this.props.mainState.rnd.products];
      let maPhieu = this.props.mainState.rnd.ma_rnd || '';
    ...
    if(record.id) {
      ...
      .then((json) => {
        if(json.status == 'error') {
          ...
          let newData = this.props.mainState.rnd.products.filter((item) => item.key != record.id);
          this.props.dispatch(updateStateData({
            rnd: {
              ...this.props.mainState.rnd,
              products: newData
      ...
      if(record.key) {
        let newData = this.props.mainState.rnd.products.filter((item) => item.key != record.key);
        this.props.dispatch(updateStateData({
          rnd: {
            ...this.props.mainState.rnd,
            products: newData
  ...
  fetchProduct() {
    fetch(ISD_BASE_URL + 'rnd/fetchProductDetailsList', {
      headers: getTokenHeader()
    })
  ...
  fetchSelectedProduct() {
    ...
    let maPhieu = rnd.ma_rnd;
    this.setState({loadProduct: true});
    fetch(ISD_BASE_URL + `rnd/fetchSelectedProduct/${maPhieu}`, {
      headers: getTokenHeader()
    })
    .then((resopnse) => resopnse.json())
    .then((json) => {
  ...
  componentDidMount() {
    let {products, rnd} = this.props.mainState;
    if(!products.length) {
      this.fetchProduct();
    }
    if(rnd.ma_rnd) {
  ...
  render() {
    ...
    let selectedProducts = this.props.mainState.rnd.products || [];
    const rowSelection = {
